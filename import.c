...
{
	int i, c, clevel = -1, flag = 0;
	char *fn_ref = 0, moder[8];
	htsFile *in;
	bcf_atombuf_t *fp;
	const bcf_atom_t *a;
	...
	if (argc - optind < 1) {
		fprintf(stderr, "Usage: bgt atomize [options] <in.bcf>|<in.vcf>|<in.vcf.gz>\n");
		fprintf(stderr, "Options:\n");
		fprintf(stderr, "  -S           input is VCF\n");
		fprintf(stderr, "  -l INT       compression level [%d]\n", clevel);
		fprintf(stderr, "  -t FILE      list of reference names and lengths [null]\n");
	...
	fp = bcf_atombuf_init(in);
	while ((a = bcf_atom_read(fp)) != 0) {
		printf("%s\t%d\t%s\t%s", fp->h->id[BCF_DT_CTG][a->rid].key, a->pos, a->ref.s, a->alt);
		for (i = 0; i < a->n_gt; ++i) {
			putchar('\t');
			putchar('0' + a->gt[i]);
		}
